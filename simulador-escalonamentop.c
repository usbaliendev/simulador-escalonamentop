#include <stdio.h>
#include <stdlib.h>

/*Variaveis*/
struct processos {
  int id;                                          
  int tduracao;               
  int prioridade;           
  int texecucao;             
  int tespera;                   
  struct processos *prox;
};

/*Declaração das funções*/
struct processos *init_processos (int id, int tduracao, int prioridade);
void fifo (struct processos *proc);
void listprocs (struct processos *proc);
void prioridade (struct processos *proc);
void rr (struct processos *proc, int quantum);
void sjf (struct processos *proc);

/*MAIN*/
int main (void) {
  struct processos *proclistado, *ptmp;
  proclistado = init_processos(1, 10, 3);
  proclistado->prox = init_processos(2,  1, 1); ptmp = proclistado->prox;
  ptmp->prox  = init_processos(3,  2, 3); ptmp = ptmp->prox;
  ptmp->prox  = init_processos(4,  1, 4); ptmp = ptmp->prox;
  ptmp->prox  = init_processos(5,  5, 2);
  listprocs(proclistado);
  fifo(proclistado);
  sjf(proclistado);     
  prioridade(proclistado);
  rr(proclistado,0);
  while (proclistado != NULL) {
    ptmp = proclistado;
    proclistado = proclistado->prox;
    free(ptmp);
  };
  return(0);
};

/* Inicialização de entrada da lista de processos*/
struct processos *init_processos (int id, int tduracao, int prioridade) {
  struct processos *proc;
  proc = (struct processos*)malloc(sizeof(struct processos)); 
  if (proc == NULL) {
    printf("Erro Fatal: Falha Alocacao de memoria.\nFinalizar.\n");
    exit(1);
  };
  proc->id = id;
  proc->tduracao = tduracao;
  proc->prioridade = prioridade;
  proc->texecucao = 0;
  proc->tespera = 0;
  proc->prox = NULL;
  return(proc);
};

/* Listando Processos*/
void listprocs (struct processos *proc) {
  struct processos *tmp = proc;
  printf("\tListagem de Processos\n");
  printf("\n");
  while (tmp != NULL) {
    printf("Processo: %d\tPrioridade: %d\tDuração: %d\n", tmp->id, tmp->prioridade, tmp->tduracao);
    tmp = tmp->prox;
  };
  printf("\n\n");
};

/*FIFO - First in First Out*/
void fifo (struct processos *proc) {
  int tempo = 0, inicio, fim;
  struct processos *tmp = proc;
  printf("\tEscalonamento FIFO\n\n");
  while (tmp != NULL) {
    inicio = tempo;
    tempo += tmp->tduracao;
    fim = tempo;
    printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", tmp->id, tempo, inicio, fim);
    tmp = tmp->prox;
  };
  printf("\n\n");
};

/*Processos por Prioridade*/
void prioridade (struct processos *proc) {
  int tempo, inicio, fim, maior;
  struct processos *copia, *tmpsrc, *tmp, *maiorprimeiro;
  printf("\tEscalonamento por Prioridade\n");
  printf("\n");
  /* Replicando a lista de processos*/
  tmpsrc = proc;
  copia = tmp = NULL;
  while (tmpsrc != NULL) {
    if (copia == NULL) {
      copia = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = copia;
    } else {
      tmp->prox = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = tmp->prox;
    };
    tmpsrc = tmpsrc->prox;
  };
  /*Codigo principal*/
  tempo = 0;
  while (copia != NULL) {
    /* Localiza o proximo processo*/
    maiorprimeiro = NULL;
    maior = copia->prioridade;
    tmp = copia->prox;
    tmpsrc = copia;
    while (tmp != NULL) {
      if (tmp->prioridade < maior) {
        maior = tmp->prioridade;
        maiorprimeiro = tmpsrc;
      };
      tmpsrc = tmp;
      tmp = tmp->prox;
    };
    if (maiorprimeiro == NULL) {
    /* Verifica se o primeiro processo possui maior prioridade*/
      inicio = tempo;
      tempo += copia->tduracao;
      fim = tempo;
      printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", copia->id, tempo, inicio, fim);
      tmpsrc = copia->prox;
      free(copia);
      copia = tmpsrc;
    } else {
    /* Verifica se o primeiro processo não possui maior prioridade*/
      tmp = maiorprimeiro->prox;
      inicio = tempo;
      tempo += tmp->tduracao;
      fim = tempo;
      printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", tmp->id, tempo, inicio, fim);
      maiorprimeiro->prox = tmp->prox;
      free(tmp);
    };
  };
  printf("\n\n");
};

/*RR - Round-Robin*/
void rr (struct processos *proc, int quantum) {
  int jobsremain, passes;
  struct processos *copia, *tmpsrc, *tmp, *slot;
  printf("\tEscalonamento Round-Robin - Quantum: %d)\n", quantum);
  printf("\n");
  tmpsrc = proc;
  copia = tmp = NULL;
  while (tmpsrc != NULL) {
    if (copia == NULL) {
      copia = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = copia;
    } else {
      tmp->prox = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = tmp->prox;
    };
    tmpsrc = tmpsrc->prox;
  };
  /*Processo de análise de prioridade*/
  jobsremain = 2;
  slot = NULL;
  while (jobsremain) {
    /* Seleciona o próximo processo efetuando sua alocação*/
    jobsremain = 0;
    if (slot == NULL) {
      slot = copia;
      jobsremain = 2;
    } else {
      passes = 0;
      do {
        if (slot->prox == NULL) {
          passes++;
          slot = copia;
        } else {
          slot = slot->prox;
        };
      } while (passes <= 3 && slot->tduracao == slot->texecucao);
      if (passes <= 3) {
        jobsremain = 2;
      };
    };
    /* Executa um ciclo*/
    tmp = copia;
    while (tmp != NULL) {
    if (tmp->tduracao > tmp->texecucao) {
      if (tmp == slot) {
        tmp->texecucao += quantum;
      } else {
        tmp->tespera += quantum;
      };
    };
    tmp = tmp->prox;
    };
  };
  /*Resultados sem as replicações*/
  tmp = copia;
  while (tmp != NULL) {
    printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", tmp->id, tmp->tduracao, tmp->tespera, tmp->texecucao + tmp->tespera);
    tmpsrc = tmp;
    tmp = tmp->prox;
    free(tmpsrc);
  };
  printf("\n");
};

/*SJF - Smallest Job First*/
void sjf (struct processos *proc) {
  int tempo, inicio, fim, shortest;
  struct processos *copia, *tmpsrc, *tmp, *beforeshortest;
  printf("\tEscalonamento SJF\n");
  printf("\n");
  /* Lista de processos é replicada*/
  tmpsrc = proc;
  copia = tmp = NULL;
  while (tmpsrc != NULL) {
    if (copia == NULL) {
      copia = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = copia;
    } else {
      tmp->prox = init_processos(tmpsrc->id, tmpsrc->tduracao, tmpsrc->prioridade);
      tmp = tmp->prox;
    };
    tmpsrc = tmpsrc->prox;
  };
  tempo = 0;
  while (copia != NULL) {
    /* Encontra o proximo processo*/
    beforeshortest = NULL;
    shortest = copia->tduracao;
    tmp = copia->prox;
    tmpsrc = copia;
    while (tmp != NULL) {
      if (tmp->tduracao < shortest) {
        shortest = tmp->tduracao;
        beforeshortest = tmpsrc;
      };
      tmpsrc = tmp;
      tmp = tmp->prox;
    };
    /* Executa processo e remove réplica da lista de processos */
    if (beforeshortest == NULL) {
      /* Aloca o primeiro processo caso o mesmo seja menor */
      inicio = tempo;
      tempo += copia->tduracao;
      fim = tempo;
      printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", copia->id, tempo, inicio, fim);
      tmpsrc = copia;
      copia = copia->prox;
      free(tmpsrc);
    } else {
      /* Aloca o primeiro processo caso não haja ocorrencia de outro menor */
      tmp = beforeshortest->prox;
      inicio = tempo;
      tempo += tmp->tduracao;
      fim = tempo;
      printf("Processo: %d\tDuração: %d\tEspera: %d\tRetorno: %d\n", tmp->id, tempo, inicio, fim);
      beforeshortest->prox = tmp->prox;
      free(tmp);
    }
  }
  printf("\n\n\n\tAperte qualquer tecla para execultar o escalonamento RR\n\n\n");
  printf("\n");
  system ("pause");
}